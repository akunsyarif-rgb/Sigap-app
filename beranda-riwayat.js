...
// sejak catatan dibuat — aturan sebenarnya ditegakkan di server).
       ...
       function LogTab({ allLogs, pelanggaranList, suratList, initialCategory, canManage, isAdmin, onEditEntry, onDeleteEntry }) {
           ...
           // sudah lewat 5 menit, supaya BK/Kesiswaan tahu fiturnya ADA tapi lagi
           // dibatasi waktu — tanpa ini, ikon abu-abu yang tidak merespons tap
           // terlihat seperti rusak, bukan sengaja dibatasi (title/tooltip tidak
           // muncul di HP lewat tap).
           const showRestrictedMsg = () => {
               setRestrictedMsg('Sudah lewat 5 menit sejak dicatat — hanya admin yang bisa ubah/hapus catatan ini.');
                                               ...
                                               <span className="flex items-center gap-2">
                                                   ...
                                                   {canManage && (
                                                       <span className="flex items-center gap-0.5">
                                                           <button
                                                               onClick={(e) => { e.stopPropagation(); if (editable) openManage(item); else showRestrictedMsg(); }}
                                                           ...
                                                           <button
                                                               onClick={(e) => { e.stopPropagation(); if (editable) { setManageMsg(''); setConfirmDeleteTarget(item); } else showRestrictedMsg(); }}
