...
// grafik batang, dll), plus layar Login, Header, dan Bottom Navigation.
       ...
       // Kolom password + tombol mata lihat/sembunyikan (masukan survei guru,
       // September 2026). Semua prop diteruskan apa adanya ke <input>, kecuali
       // `type` yang dikendalikan di sini: bawaan TERSEMBUNYI. pr-12 memberi
       // ruang kanan supaya teks tidak tertimpa ikon. Tombolnya type="button"
       // (bukan submit) supaya menekan mata tidak ikut mengirim form login.
       // Sengaja TIDAK memakai preventDefault di onTouchStart: di sebagian HP
       // itu membatalkan klik, dan React memasang listener touch sebagai pasif.
       function PasswordField({ className = '', ...inputProps }) {
           ...
           return (
               <div className="relative">
                   ...
                   <button
                       ...
                       className="absolute inset-y-0 right-0 w-12 flex items-center justify-center text-slate-500 hover:text-slate-700 focus:outline-none focus:text-sky-dim"
                   >
                       <Icon
                           className="h-5 w-5"
                           path={visible
                               ? <path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" />
                               : <React.Fragment><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></React.Fragment>}
                       />
                   </button>
               </div>
           );
           ...
           return (
               <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                   <form onSubmit={submit} className="bg-white w-full max-w-sm rounded-3xl p-6 border border-slate-200 shadow-2xl space-y-4 animate-pop">
                       ...
                       <PasswordField autoComplete="current-password" value={oldPassword} onChange={(e) => setOldPassword(e.target.value)} placeholder="Password lama" className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-sky" />
                       <PasswordField autoComplete="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Password baru (min. 6 karakter)" className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-sky" />
                       <PasswordField autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Konfirmasi password baru" className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5 text-xs text-slate-900 focus:outline-none focus:border-sky" />
